 ...
 * <p/>
 * This class provides CRUD functionality for all Invoice entities. It focuses purely on Java EE 6 standards (e.g. <tt>&#64;ConversationScoped</tt> for state
...
public class InvoiceBean implements Serializable {
    ...
    public void paginate() {
        ...
        TypedQuery<Invoice> query = this.entityManager.createQuery(criteria.select(root).where(getSearchPredicates(root)));
    ...
    /*
     * Support listing and POSTing back Invoice entities (e.g. from inside an HtmlSelectOneMenu)
     */

    public List<Invoice> getAll() {

        CriteriaQuery<Invoice> criteria = this.entityManager.getCriteriaBuilder().createQuery(Invoice.class);
        return this.entityManager.createQuery(criteria.select(criteria.from(Invoice.class))).getResultList();
    }
    ...
    public Converter getConverter() {
        ...
        final InvoiceBean ejbProxy = this.sessionContext.getBusinessObject(InvoiceBean.class);
