...
public class BookEndpointTest {
    ...
    public static WebArchive createDeployment() {
        return ShrinkWrap.create(WebArchive.class).addClass(RestApplication.class).addClass(BookEndpoint.class).addClass(Book.class).addClass(Item.class)
                .addClass(Language.class).addClass(LanguageConverter.class).addClass(Category.class).addClass(Publisher.class).addClass(Artist.class)
                .addClass(Author.class).addClass(ResourceProducer.class).addAsResource("META-INF/persistence-test.xml", "META-INF/persistence.xml")
    ...
    public void should_be_deployed() {
        ...
        final WebTarget target = client.target(baseURL).path("rest").path("books");
    ...
    public void shouldCRUDBooks() throws IOException {
        final Client client = ClientBuilder.newClient();
        final WebTarget target = client.target(baseURL).path("rest").path("books");
        final int initialSize = target.request(MediaType.APPLICATION_XML).get(Books.class).size();

        // creates a book
        final Book book = new Book("1234-5678", "H2G2", "The best Scifi book", 45.5f, 345, Language.ENGLISH);
        Response response = target.request().post(Entity.entity(book, MediaType.APPLICATION_XML));
        assertEquals(201, response.getStatus());
        final URI locationNewBook = response.getLocation();
