...
public class BookMarshallerTest {
    ...
    public void should_marshall() throws JAXBException {
        final Book book = new Book("1234", "H2G2", "Best Scifi book", 63.98F, 123, Language.ENGLISH);
    ...
    public void should_buid_book_json() {
        final JsonObject jsonBook = Json.createObjectBuilder()
                .add("book",
                        Json.createObjectBuilder().add("isbn", "1234").add("title", "H2G2").add("unit-cost", "63.98").add("nb-of-pages", "123")
                                .add("description", "Best Scifi book").add("language", "ENGLISH").add("category", Json.createObjectBuilder().add("name", "IT"))
                                .add("publisher", Json.createObjectBuilder().add("name", "Megadodo"))
                                .add("authors", Json.createArrayBuilder().add(Json.createObjectBuilder().add("first-name", "Douglas").add("last-name", "Adams"))
                                        .add(Json.createObjectBuilder().add("first-name", "John").add("last-name", "Cleese"))))
                .build();
    ...
    public void should_process_books() throws IOException {
        ...
        final InputStream stream = Files.newInputStream(Paths.get("src/test/resources/books.json"));
